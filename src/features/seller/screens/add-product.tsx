...
import { Icon, Button, Spinner, Price, EmptyState, AppLink } from "@/components/ui";
import {
  FormSectionNav,
  FormActionBar,
  MessageBar,
...
// exact (per SKU) → option (per attribute value) → product main image. The
...
export function SellerAddProduct({
  ...
  // attribute value (option), or a per-SKU exact image. Drives the server's
  ...
  // Structured PDP fields — warranty, brand, SKU. Shown on the product page as
  ...
  const activeSection = useScrollSpy(navIds, "app-scroll");
  const currentNavIdx = Math.max(0, navIds.indexOf(activeSection ?? "sec-media"));
  ...
  const sections: FormSection[] = [
    ...SECTION_DEFS.map((s): FormSection => {
      ...
      else if (s.id === activeSection || sectionTouched[s.key]) state = "active";
      else state = "todo";
  ...
  if (!categoryOk)
    validationItems.push({ section: "Product details", id: "sec-basics", msg: "Choose a category." });
  if (hasVariants ? !variantsOk : !(price && stock))
    validationItems.push({
      section: "Variants & pricing",
      id: "sec-variants",
      msg: hasVariants ? "Every variant needs a price and stock." : "Set a price and stock.",
    });
  if (!saleValid)
    validationItems.push({
      section: "Variants & pricing",
  ...
  const draft = useLocalDraft<ProductDraft>("bz-draft-add-product");
  ...
  const formPristine =
    !title && !description && !category && !price && !stock && !hasVariants && productPhotos.length === 0;
  // On first mount, offer to restore a previous draft if the form is empty.
  useEffect(() => {
    if (!draftEnabled || draftChecked.current) return;
    draftChecked.current = true;
    if (formPristine && draft.exists()) setShowRestore(true);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  // Debounced autosave once the seller starts filling the form.
  useEffect(() => {
    if (!draftEnabled || showRestore || formPristine) return;
    const handle = setTimeout(() => draft.write(buildDraft()), 600);
  ...
  const addPresetAttribute = (preset: string) => {
    ...
    if (
      name &&
      variantGroupDefs.some((g) => g.name.trim().toLowerCase() === name.toLowerCase())
    ) {
      return;
    }
      ...
      ? variants
          ...
          .map((v) => {
            ...
            return {
              ...
              // Only "exact" mode stores a per-SKU image; option/product modes
  ...
  return (
    <div className="bz-seller-page">
      <div className="bz-seller-add-layout">
        <FormSectionNav sections={sections} activeId={activeSection} onJump={scrollToSection} />
        <div className="bz-seller-add-form">
          ...
          {draftEnabled && showRestore && (
            <MessageBar
              tone="info"
              title="You have an unsaved local draft"
            ...
            >
              Saved on this device while you were filling the form. Photos aren&rsquo;t included —
              you&rsquo;ll need to re-add them.
            </MessageBar>
          )}
          ...
          {/* Warranty — shown on the product page as first-class trust info
              (backend-driven, not buried in specs). */}
          {category && (
            <div
              style={{
                background: "#fff",
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                ...
                <h3 style={{ margin: 0, fontSize: "1rem", fontWeight: 800 }}>Warranty</h3>
              </div>
              ...
              >
                ...
                <div>
                  <label style={pdpLabelStyle}>
                    Seller SKU (optional)
                    <InfoTip text="Your own stock code for this product. BazaarCo generates the platform SKU automatically — this is just for your records, so it's optional." />
                  </label>
                  <input
                    value={sku}
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                <h3 style={{ margin: 0, fontSize: "1rem", fontWeight: 800 }}>
                  ...
                  <InfoTip text="A variant is one exact sellable version with its own price, stock and SKU — e.g. Red / Medium. Turn this on when the same product sells in different options." />
                </h3>
              </div>
            </div>

            {/* Product type — does this product come in variants? Two cards, like
                the demo's "No options / Has options" choice. */}
            <p style={{ margin: "0 0 6px", fontSize: ".8125rem", color: "var(--ink-500)" }}>
              Does this product have variants?
            </p>
            <p style={{ margin: "0 0 8px", fontSize: ".75rem", color: "var(--ink-400)" }}>
              Examples — T-shirt: Size (S/M/L) &amp; Color. iPhone: Storage (128/256GB) &amp;
              Color. Jacket: Size &amp; Material. No options? Keep it a single product.
            </p>
            <div
              style={{
                display: "grid",
            ...
            >
              {(
                [
                  [false, "No variants", "One price and stock for the whole product."],
                  [true, "Has variants", "Sizes, colors, storage, design, and more."],
                ] as const
            ...
            {!hasVariants ? (
              ...
              <div>
                ...
                <p style={{ margin: "0 0 12px", fontSize: ".8125rem", color: "var(--ink-500)" }}>
                  Add the options your product comes in. Pick an option type, list its
                  choices, and BazaarCo builds a row for every combination automatically.
                </p>

                {/* Quick-add option types */}
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>
                  {VARIANT_OPTION_BUTTONS.map((preset) => {
                    const already =
                      preset !== "Custom" &&
                ...
                {/* Variant table actions — mirrors the demo's button row. */}
                {variantGroupDefs.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>
                    <Button
                      variant="ghost"
                      size="sm"
                      icon="refresh"
                      onClick={() => syncMultiVariants(variantGroupDefs)}
                    >
                      Regenerate variant table
                    </Button>
                    <Button variant="ghost" size="sm" icon="trash" onClick={() => chooseHasVariants(true)}>
                      Clear variants
                    </Button>
                  </div>
                )}

                {/* How variant photos resolve */}
                {variants.length > 0 && (
                  <div
                    style={{
                      border: "1px solid var(--line-200)",
                  ...
                  >
                    ...
                    >
                      How do product photos change?
                      <InfoTip text="Three image levels: product (one main image for all), option (one image per color/style — e.g. the Red photo covers Red/S, Red/M, Red/L), or exact (a separate image for every single variant)." />
                    </p>
                    <p style={{ margin: "0 0 10px", fontSize: ".75rem", color: "var(--ink-500)" }}>
                      Each variant uses its exact image if set, then its option image, then the
                      main product image.
                    </p>
                      ...
                      {(
                        [
                          [
                            "product",
                            "Use main image",
                            "Fastest — every variant shows the main product photo.",
                          ],
                          [
                            "option",
                            "Image by option",
                            "One photo per choice of an option, e.g. one per Color.",
                          ],
                          ["exact", "Exact per variant", "Upload a separate photo for each row below."],
                        ] as const
                      ).map(([mode, label, desc]) => {
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    {uploading ? (
                                      ...
                                      <Icon name="camera" size={16} color="var(--ink-400)" />
                                    )}
                                  </label>
                                </div>
                              );
                ...
                {variants.length > 0 && (
                  <div>
                    ...
                    >
                      {variants.length} variant{variants.length !== 1 ? "s" : ""} — set price &amp;
                      stock for each:
                    </p>
                    <p style={{ margin: "-4px 0 8px", fontSize: ".75rem", color: "var(--ink-400)" }}>
                      Each row is one exact sellable variant with its own stock, price
                      {variantImageMode !== "product" ? ", image" : ""} and SKU.
                    </p>
                    <div
                      style={{
                        border: "1px solid var(--line-200)",
                    ...
                    >
                      {(() => {
                        ...
                        const gridCols = showImg ? "44px 1fr 104px 80px" : "1fr 104px 80px";
                        return (
                          <>
                            <div
                              style={{
                            ...
                            >
                              ...
                              <span
                                style={{ fontSize: ".75rem", fontWeight: 700, color: "var(--ink-600)" }}
                              >
                                Variant
                              </span>
                              <span
                                    ...
                                    (variantImageMode === "exact" ? (
                                      ...
                                      >
                                        {optionImg ? (
                                          ...
                                          <Icon name="image" size={14} color="var(--ink-300, #ccc)" />
                                        )}
                                      </div>
                                    ))}
                                  <span
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
              ...
              >
                {canPublish ? <Icon name="check" size={18} color="#fff" /> : 5}
              </span>
              <div>
                <h3 style={{ margin: 0, fontSize: "1rem", fontWeight: 800 }}>
            ...
            >
              {[
                ...
                { ok: categoryOk, label: "Category selected", id: "sec-basics" },
                {
                  ...
                  label: hasVariants ? "Every variant has price & stock" : "Price & stock added",
                  id: "sec-variants",
                },
                ...
                <li key={c.label} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  >
                    {c.ok ? <Icon name="check" size={13} /> : <Icon name="clock" size={12} />}
                  </span>
                  <span
                    style={{
                      flex: 1,
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              ...
              >
                {mainPhoto[0] ? (
                  ...
                  <Icon name="image" size={24} color="var(--ink-300)" />
                )}
              </div>
              <div
                style={{
              ...
              >
                {[
                  ...
                  { label: "Seller SKU", value: sku || "—", id: "sec-basics" },
                ].map((r) => (
                  <div key={r.label}>
                    <div
                      style={{
            ...
            right={
              <>
                <Button
                  ...
                  onClick={() => goToNav(currentNavIdx - 1)}
                >
                  Previous
                </Button>
                <Button
                  ...
                  disabled={currentNavIdx >= navIds.length - 1}
                  onClick={() => goToNav(currentNavIdx + 1)}
                >
                  Next
                </Button>
