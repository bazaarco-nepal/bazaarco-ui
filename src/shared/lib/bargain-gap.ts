...
// without a round-trip. Keep it in sync with the backend default (3). It is a
...
export const BARGAIN_MIN_GAP_PERCENT =
  Number.isFinite(PARSED) && PARSED > 0 && PARSED < 100 ? PARSED : 3;

/**
 * Highest floor a seller may set for a listed price, floored so the gap is always
 * at least BARGAIN_MIN_GAP_PERCENT — i.e. the floor must be ≤ (100 − gap)% of
 * listed. Matches the backend's getMaxAllowedBargainMinimum.
